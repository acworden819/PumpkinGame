...
//Avatar
var wiz = new GameObject({width:128*1.5, height:128*1.5, spriteData:playerData}).makeSprite(playerData)
wiz.force=1
...
var cave = new Grid(caveData, {world:level, x:0, tileHeight:64, tileWidth:64});
//Cave background Tile Grid
var caveBack = new Grid(caveBackData, {world:level, x:0, tileHeight:64, tileWidth:64});
//cave hitbox grid
var caveHit = new Grid(caveHitData, {world:level, x:0, tileHeight:64, tileWidth:64});
...
gameStates[`level1`] = function()
{
	...
	if(keys[` `] )
	{
		if(canShoot)
		{
			...
			bullets[currentBullet].x = wiz.x-level.x + (wiz.dir * 90) ;
